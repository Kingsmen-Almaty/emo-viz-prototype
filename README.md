...
The GET endpoints do not run new inference and do not return the AI Trail sentiment copy such as `Excited` or `Concerned`. They are lightweight raw-data taps that return the most recent YuNet feature-placement payload and FER+ emotion payload generated by the live camera pipeline. External apps can poll them without sending images.
